...
console.log('🔧 Staging Server Environment Check:');
...
const app = express();
const PORT = process.env.PORT || 63037;
...
app.post('/migrate-db', requireApiKey, async (req, res) => {
  try {
    ...
    console.log(`🚀 Starting database migration from staging to production... (initiated by admin: ${email})`);

    // This is the staging server, so we migrate FROM staging TO production
    const stagingUri = 'mongodb://localhost:27017/bolola-staging';
    const productionUri = 'mongodb://localhost:27017/bolola-production';

    console.log(`📤 Source (staging): ${stagingUri}`);
    console.log(`📥 Target (production): ${productionUri}`);

    // Create separate connections for migration
    const mongoose = require('mongoose');

    // Source connection (staging)
    const sourceConnection = mongoose.createConnection(stagingUri);
    await sourceConnection.asPromise();

    // Target connection (production)
    const targetConnection = mongoose.createConnection(productionUri);
    ...
    console.log('Source (staging):', sourceCounts);
    console.log('Target (production):', initialTargetCounts);
    ...
    const migrateCollection = async (sourceModel, targetModel, collectionName) => {
      ...
      console.log(`   Found ${documents.length} documents in staging`);
      ...
      console.log(`   🗑️  Cleared ${deleteResult.deletedCount} documents from production`);
    ...
    console.log('\n📊 Final production counts:', finalTargetCounts);
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`MongoDB-based staging server running at http://0.0.0.0:${PORT}`);
}); 